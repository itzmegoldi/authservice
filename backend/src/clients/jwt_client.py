...
class JwtClient:
    def __init__(self, config: Config) -> None:
        ...
        if private_pem:
            ...
            self.public_key = serialization.load_pem_public_key(public_pem.encode()) if public_pem else self.private_key.public_key()
        else:
    ...
    def create_access_token(self, user: UserAccount) -> tuple[str, datetime, list[str]]:
        ...
        token = jwt.encode(claims, self._private_pem(), algorithm="RS256", headers={"kid": self.kid})
        return token, expires_at, roles

    def decode_access_token(self, token: str) -> dict[str, Any]:
        return jwt.decode(token, self._public_pem(), algorithms=["RS256"], issuer=self.config.auth.issuer)
